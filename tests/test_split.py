...
def test_splitter_preserves_species_order_in_output(tmp_path):
    xml = '''<?xml version="1.0" encoding="UTF-8"?>
<orthoXML xmlns="http://orthoXML.org/2011/">
  <species name="s1">
    <database name="db1">
...
'''
    ...
    out_file = outdir / "HOG_1.orthoxml"
    tree = etree.parse(str(out_file), etree.XMLParser(remove_blank_text=True))
    species = tree.findall("{http://orthoXML.org/2011/}species")
    assert [sp.get("name") for sp in species] == ["s1", "s2"]
